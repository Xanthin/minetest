	...
	: Mapgen(mapgenid, params, emerge)
{
	...
	noise_valley1      = new Noise(&sp->np_valley1,     seed, csize.X, csize.Z);
	noise_valley2      = new Noise(&sp->np_valley2,     seed, csize.X, csize.Z);
	noise_valley_amp   = new Noise(&sp->np_valley_amp,  seed, csize.X, csize.Z);
	noise_hill         = new Noise(&sp->np_hill,        seed, csize.X, csize.Z);
	...
	noise_cave4    = new Noise(&sp->np_cave4,    seed, csize.X, csize.Y + 2, csize.Z);
...
MapgenWatershed::~MapgenWatershed()
{
	...
	delete noise_valley1;
	delete noise_valley2;
...
MapgenWatershedParams::MapgenWatershedParams()
{
	...
	np_valley_base = NoiseParams(0.0,  1.0,  v3f(6144, 6144, 6144), 106,    3, 0.4,  2.0);
	np_valley1     = NoiseParams(0.0,  1.0,  v3f(2048, 2048, 2048), 2177,   5, 0.5,  2.0);
	np_valley2     = NoiseParams(0.0,  1.0,  v3f(2048, 2048, 2048), 92,     5, 0.5,  2.0);
	np_valley_amp  = NoiseParams(4.0,  4.0,  v3f(2048, 2048, 2048), 11206,  3, 0.5,  2.0);
	np_hill        = NoiseParams(0.0,  1.0,  v3f(96,   96,   96),   7165,   3, 0.4,  2.0);
	np_plateau     = NoiseParams(0.5,  0.5,  v3f(2048, 2048, 2048), 63,     5, 0.4,  2.0);

	np_mountain    = NoiseParams(0.0,  1.0,  v3f(384,  384,  384),  700334, 5, 0.7,  2.0);
	np_cave1       = NoiseParams(0.0,  1.0,  v3f(191,  191,  191),  52534,  3, 0.5,  2.0);
	np_cave2       = NoiseParams(0.0,  1.0,  v3f(193,  193,  193),  325,    3, 0.5,  2.0);
	np_cave3       = NoiseParams(0.0,  1.0,  v3f(197,  197,  197),  109848, 3, 0.5,  2.0);
	np_cave4       = NoiseParams(0.0,  1.0,  v3f(199,  199,  199),  1098,   3, 0.5,  2.0);
}
...
void MapgenWatershedParams::readParams(const Settings *settings)
{
	...
	settings->getNoiseParams("mgwatershed_np_valley1",     np_valley1);
	settings->getNoiseParams("mgwatershed_np_valley2",     np_valley2);
	settings->getNoiseParams("mgwatershed_np_valley_amp",  np_valley_amp);
	settings->getNoiseParams("mgwatershed_np_hill",        np_hill);
	...
	settings->getNoiseParams("mgwatershed_np_cave4",       np_cave4);
}
...
void MapgenWatershedParams::writeParams(Settings *settings) const
{
	...
	settings->setNoiseParams("mgwatershed_np_valley1",     np_valley1);
	settings->setNoiseParams("mgwatershed_np_valley2",     np_valley2);
	settings->setNoiseParams("mgwatershed_np_valley_amp",  np_valley_amp);
	settings->setNoiseParams("mgwatershed_np_hill",        np_hill);
	...
	settings->setNoiseParams("mgwatershed_np_cave4",       np_cave4);
}
...
int MapgenWatershed::getGroundLevelAtPoint(v2s16 p)
{
	...
	float n_valley = fabs(NoisePerlin2D(&noise_valley1->np, p.X, p.Y, seed) *
		NoisePerlin2D(&noise_valley2->np, p.X, p.Y, seed));
	...
	for (s16 y = water_level; y <= water_level + 128; y++) {
		float n_mountain = (1.0 + NoisePerlin3D(&noise_mountain->np, p.X, y, p.Y, seed))
			* mountain_amp;
		float density_gradient = -y / MGWATERSHED_VERTICAL_SCALE;
...
void MapgenWatershed::calculateNoise()
{
	...
	noise_valley1->perlinMap2D(x, z);
	noise_valley2->perlinMap2D(x, z);
	noise_valley_amp->perlinMap2D(x, z);
	noise_hill->perlinMap2D(x, z);
...
int MapgenWatershed::generateTerrain()
{
	...
	for (s16 x = node_min.X; x <= node_max.X; x++, index2d++) {
		// large scale tectonic mountain ridges structure
		// also roughly river surface
		float n_valley_base = -pow(fabs(noise_valley_base->result[index2d]), 0.8);
		// river valley structure
		float n_valley = fabs(noise_valley1->result[index2d] *
			noise_valley2->result[index2d]);
		// vary amplitude of valley sides
		float n_valley_amp = MYMAX(noise_valley_amp->result[index2d], 0.0);
		// chop valley sides at a varying plateau height
		float n_plateau = MYMAX(noise_plateau->result[index2d], 0.0);

		// valley cross section sinks a little below 0 to define river area/width
		n_valley += n_valley_base * 0.04;

		// blend = 0 for river and smooth valleys/plateaus, blend = 1 for mountains
		// initialise blend to zero for river
		float blend = 0.0;
		float mountain_amp = n_valley * MGWATERSHED_MOUNTAIN_AMP;
		if (n_valley > 0.0) {
			// shape valley with exponent
			n_valley = pow(n_valley, 1.5);
			// add small hills
			n_valley += n_valley * noise_hill->result[index2d] * 0.2;
			// scale and chop valley at plateau
			n_valley = MYMIN(n_valley * n_valley_amp, n_plateau);
			// cosine blend with mountain noise according to distance from ridge
			if (n_valley_base > -0.1)
				// mountains
				blend = 1.0;
			else if (n_valley_base > -0.3)
				// transition area
				blend = (cos((-0.1 - n_valley_base) * 15.707963) + 1.0) / 2.0;
		} else {
			// river channel
			// make n_valley positive to allow fractional exponent
			// create river cross section shape and multiply for depth
			n_valley = -pow(-n_valley, 0.2) * 0.1;
		}

		// apply ridge height and valley base amplitude before y loop
		n_valley_base = MGWATERSHED_RIDGE_HEIGHT +
			n_valley_base * MGWATERSHED_VALLEY_BASE_AMP;
		// apply blend to n_valley before y loop
		float n_valley_blend = n_valley * (1.0 - blend);

		u32 i = vm->m_area.index(x, node_min.Y - 1, z);
		u32 index3d = (z - node_min.Z) * zstride + (x - node_min.X);
		for (s16 y = node_min.Y - 1; y <= node_max.Y + 1; y++) {
			float n_mountain = (2.0 + noise_mountain->result[index3d]) * mountain_amp;
			float density_gradient = -y / MGWATERSHED_VERTICAL_SCALE;
			// actual valley base, roughly river surface level
			float density_valley_base = n_valley_base + density_gradient;
			// actual solid terrain level
			float density = density_valley_base + n_valley_blend + n_mountain * blend;
...
void MapgenWatershed::generateCaves(int max_stone_y)
{
	if (max_stone_y >= node_min.Y) {
		...
		for (s16 y = node_min.Y - 1; y <= node_max.Y + 1; y++) {
			...
			float cavem_off = -0.00015 * (y - MGWATERSHED_LAVA_CAVE_LEVEL) *
				(y - MGWATERSHED_LAVA_CAVE_LEVEL);
			u32 ni2d = 0;
			for (s16 z = node_min.Z; z <= node_max.Z; z++) {
				u32 ni3d = (z - node_min.Z) * zstride + (y - node_min.Y + 1) * ystride;
				u32 vi = vm->m_area.index(node_min.X, y, z);
				for (s16 x = node_min.X; x <= node_max.X; x++, vi++, ni2d++, ni3d++) {
					// Check n_valley to keep tunnels away from rivers
					float n_valley = fabs(noise_valley1->result[ni2d] *
						noise_valley2->result[ni2d]);
					...
					float nabs_cave4 = fabs(noise_cave4->result[ni3d]);
					...
					bool is_cave =
						...
						nabs_cave4 * cave_amp > MGWATERSHED_CAVE_THRESHOLD;
					bool is_tunnel =
						nabs_cave1 + nabs_cave2 < MGWATERSHED_TUNNEL_WIDTH ||
						nabs_cave2 + nabs_cave3 < MGWATERSHED_TUNNEL_WIDTH ||
						nabs_cave3 + nabs_cave4 < MGWATERSHED_TUNNEL_WIDTH ||
						nabs_cave4 + nabs_cave1 < MGWATERSHED_TUNNEL_WIDTH;
